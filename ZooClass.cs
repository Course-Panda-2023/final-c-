...
namespace Zoo
{
    internal class ZooClass
    {
        ...
        private const int _price = 50, _priceWithFeeder = 100, _priceWithCleaner = 30, _priceWithDoctor = 20;
        ...
        public void TimeElapsed(/*object sender, ElapsedEventArgs e*/) 
        {
            ...
            Manager.Instance.WriteToFile($"Total profits for the day : {_profits}");
            return;
        ...
        public bool CanTourInZone(ZoneType zone)
        {
            ...
            //        return false; 
            //}
            if(ActiveCleanersInZone(zone))
                return false;
            if(ActiveDoctorsInZone(zone))
                return false;
            return true;
        }

        public bool ActiveCleanersInZone(ZoneType zone)
        {
            foreach (Cleaner c in _cleaners)
            {
                if (c.Zone == zone)
        ...
        public bool ActiveDoctorsInZone(ZoneType zone)
        {
            foreach (Doctor d in _doctors)
            {
                if (d.Zone == zone)
        ...
        public bool ActiveFeederInZone(ZoneType zone)
        {
            foreach (Feeder f in _feeders)
            {
                if (f.Zone == zone)
        ...
        public void CheckTours()
        {
            ...
            if( _currTime + _tourTime <= _day)
            {
                if(_waitingToStart == null)
                {
                    ...
                    BuyTicketForTour(_waitingToStart);
                }
        ...
        public void BuyTicketForTour(Tour tour)
        {
            if (ActiveDoctorsInZone(tour.zoneType))
                _profits += tour.visitors.Count * _priceWithDoctor;
            else if (ActiveCleanersInZone(tour.zoneType))
                _profits += tour.visitors.Count * _priceWithCleaner;
            else if (ActiveFeederInZone(tour.zoneType))
                _profits += tour.visitors.Count * _priceWithFeeder;
            else
                _profits += tour.visitors.Count * _price;
